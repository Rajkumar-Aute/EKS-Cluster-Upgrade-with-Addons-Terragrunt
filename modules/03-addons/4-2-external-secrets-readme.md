...
Ensure the operator, the webhook, and the cert-controller are running in the external-secrets namespace.
...
Since you are using IRSA, the external-secrets ServiceAccount must be annotated with the AWS IAM Role ARN created by your Terraform module.
...
This is the most important check. It tells you if the operator successfully authenticated with AWS using the IAM role.
...
If it doesn't say Valid, check the error message:
