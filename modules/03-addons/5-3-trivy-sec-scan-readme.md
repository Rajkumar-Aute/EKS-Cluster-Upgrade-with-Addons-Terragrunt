...
First, ensure the operator itself is running and healthy in its own namespace.
...
When Trivy detects a pod, it spawns a temporary "Scan Job." You can see these brief jobs appearing and disappearing.
...
Run this command to create a simple deployment using an outdated, insecure image:
...
Trivy Operator uses "Informer" patterns, meaning it notices the new Pod immediately. It will schedule a temporary scan job in the trivy-system namespace.
...
If you want to see exactly which security checks failed (e.g., "Privileged" or "RunAsNonRoot"):
...
#### Step 5: Cleanup
Don't leave insecure pods running in your cluster!
...
Check the "Age" of your reports: ``` kubectl get vulnerabilityreports -A ```
