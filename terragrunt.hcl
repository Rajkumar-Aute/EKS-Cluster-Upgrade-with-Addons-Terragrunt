...
# Remote state configuration aws S3 bucket
remote_state {
  backend = "s3"
  generate = {
    path      = "backend.tf"
  ...
  config = {
    # Replace this with a globally unique bucket name (e.g., your initials + devsecops-state)
    bucket         = "devsecopsguru-eks-terraform-state-some-random-numbers"
